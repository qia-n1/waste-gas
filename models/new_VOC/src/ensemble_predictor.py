...
from .config import DataConfig, ModelConfig, SENSOR_COLUMNS
...
class EnsemblePredictor:
    """Deployment adapter for FastAPI service.

    Notes:
    - This class is intentionally tolerant to partial runtime dependencies.
    - If torch/model loading fails, it falls back to trend-based warm prediction.
    """
    ...
    def load_models(self) -> None:
        """Try loading real DLinear-Large model and scaler artifacts."""
        try:
            import torch
            from .features import VOCSFeaturePipeline
            from .model import DLinearForecasterLarge

            base_dir = Path(__file__).resolve().parents[1]
            model_candidates = [
                base_dir / "artifacts_pca_ensemble" / "pca_dlinear_large.pt",
                base_dir / "ensemble_docker" / "models" / "pca_dlinear_large.pt",
            ]
            csv_candidates = [
                base_dir.parent / "VOCS" / "src" / "data" / "vocs_dataset.csv",
                base_dir / "ensemble_docker" / "data" / "vocs_dataset.csv",
                base_dir / "src" / "data" / "vocs_dataset.csv",
            ]
